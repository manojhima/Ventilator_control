...
{
	//V�rification si d�but de page m�moire eeprom	 (page de 32 octets)
	if (Address % 32 == 0) 
		{
		...
		do
			{
			//Contr�le m�moire eeprom libre (pas d'�criture en cours)
			DRV_OutputPort(CE_EEP, 0); // Activation chip enable EEPROM
			SSCTB  = 0x0005;          // Envoi de l'instruction RDSR	 activation de l'�criture eeprom
		...
		while ((SSCRB & 0x0001)  != 0x00); 	  //v�rification si �criture en cours
		//Transmission de l'adresse ( page m�moire suivante)
		DRV_OutputPort(CE_EEP, 0); // Activation chip enable EEPROM
