...
Soit BUSCON0 = 0x00BF ;
	...
	Les broches ALE et CMD sont command�e par le �c ;

	Les donn�es envoy�es dans le XBUS par le �c sont plac�e dans un tampon,
	d'ou le �c execute la suite des instructions sans avoir fini de tranf�rer
	l'int�gralit� des donn�es ;

	Pour contrer ce temps, ajouter une tempo (_nop()) avant la retomber des
	commandes ALE et CMD.

	Cette tempo doit �tre de 1 _nop() suppl�mentaire par rapport au nombre
	de waitstate pr�sente sur le BUSCON0.
...
De plus:
	...
	- Acc�s � un pointeur en global trop long 
	- Placer les pointeurs en local
	- Xhuge non n�cessaires 
	- Pour gagner sur les temps d'acc�s pointeur, dans une custom optimisation
	cocher CSE
