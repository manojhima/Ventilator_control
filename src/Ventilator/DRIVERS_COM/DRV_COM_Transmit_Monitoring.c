...
/******************************************************************************/
/******************************************************************************/
/* -	Pression patient : de -200 � + 900  en 10-1 mbar sur deux octets : 		*/
/*		1�octet = pression / 256 et 2� octet = pression %256. Le signal de 		*/
/* 	pression est renvoy� dans un format sp�cifique 									*/
/*		permettant de simplifier la transmission des valeurs n�gatives. Un 		*/
/* 	offset de + 600 est propos� par d�faut. 											*/
/*		La prise en compte des d�cimales est n�cessaire afin d'�viter une 		*/
/* 	apparence de courbe en " escalier ".												*/

/* -	D�bit inspir� : de 0 � 2000 en dl/min sur deux octets :						*/
/* 	1�octet = d�bit / 256 et 2� octet = d�bit %256.									*/

/* -	D�bit expir� : de 0 � 2000 en dl/min sur deux octets :						*/
/*    1�octet = d�bit / 256 et 2� octet = d�bit %256.									*/

/* -	Caract�res d'alarme : sur 4 octets utilisables pour transmettre les 		*/
/* 	�tats  d'activation ou d�sactivation des alarmes 								*/
/*		(voir � Options de monitorage).														*/

/* -	Caract�re de fin de trame : sur un octet. Le caract�re de fin de trame	*/
/* 	permet d'identifier les ensembles 													*/
/*		successifs de donn�es renvoy�es � chaque interruption. Ce caract�re est	*/
/* 	aussi utilis� pour identifier 														*/
...
UWORD16 Mode_Var;
	...
	if((	TIM_ReadDecounterValue(TIM_COM_RECORDING_MONIT) == 0)
		...
		for (Alarm = 1;Alarm < Mark_Displayed_Alarm;Alarm++)
		{
			...
			/* si CANCELED et alarme sans priorit� la zone reste � ZERO */			
		...
		/*%C Autorisation de l'interruption de transmission RS232 					*/
	  	S0TBIR = 0;
		PECC2 = 0x0500; // incr�mentation du pointeur source - transfert d'un word
		PECC2 |= End_Of_Table_Com_TX_Remote + End_Of_Table_Direct_Monitoring - 1; // taille du tableau d'�v�nement � transf�rer
		SRCP2 = _sof_(&Array_Direct_Monit); 	// adresse du tableau d'�v�nement 
