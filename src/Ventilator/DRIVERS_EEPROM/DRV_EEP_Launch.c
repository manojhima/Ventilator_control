...
void DRV_EEP_Launch(void)
{
	...
	if ( DRV_EEP_StateEepDriver == DRV_EEP_WRITE_PENDING)
		{
		//test de fin d'�criture
		DRV_EEP_WaitEndWrite();
		// si fin d'�criture eeprom libre
		if ( DRV_EEP_StateEepDriver == DRV_EEP_FREE)
			{
			EEP_FIFO_FreeFirstData();
			}
		}
	// Si pas d'�criture en cours
	else
		{
		//si donn�e pr�sente dans la FIFO EEPROM
...
/* ---------------------------------------------------------------------------*/
/*		 /\																							*/	
/*    /!!\		ATTENTION, Bien VERIFIER l'adresse de d�but EEPROM dans eprom.h*/
/*   /____\																							*/
/* ---------------------------------------------------------------------------*/
