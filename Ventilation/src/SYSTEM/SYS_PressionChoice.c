...
/******************************************************************************/
 ...
 // For all modes other than CPAP.
    if ( Adjust_Mode != CPAP )
 	{	  
        ...
        if ( FilterPatientPressureProxi > maxi_prox ) 
        {
			...
			// Cas special de bloquage en expi du capteur pression interne � un niveau inf�rieur de la PEEP, menant
		   	// abusivement � une FLAT PROX=TRUE et FLAT INT=FALSE.(contraire � la r�alit�).
	...
	// Calcul du delta de pression � la transition expi-inspi et r�inintilalisation des min et max.
    // Calculate the delta pressure transition expire inspiration and r�inintilalisation of min and max.
	if (( VentilState != VEN_EXHALATION )&&( MemoState == VEN_EXHALATION ))
	 {	
		 ...
		 // Mise � jour � true du flag flat proxi si Delata < 0.3 CmH2O pour CPAP.
		 ...
		 {
		 	FlatProxi = FALSE;
		    // Mise � false temporaire des flag de detection de la high pressure
		 ...
		 // Mise � jour � true du flag flat internal si Delata < 0.3 CmH2O pour CPAP.
		 // ou si Delata < 0.4 CmH2O pour les autres modes.
		 ...
		 // R�initialisation des maxi et des mini.
         // Reset maxi and mini.
		 maxi_prox = 0x8000; 
		 mini_prox = 0x7FFF;
		 maxi_int = 0x8000; 
...
/*******************************************************************************/
// Choix du niveau de Delat Pression entre proximal et interne en fonction du d�bit pour reconnaitre une perte proximale.
...
/************************************************* *****************************/
	...
	// Si les conditions nominales de test interne vers proxi sont respect�s pendant 400 ms, on switche � l'inspi suivante.
	// Si on detecte un d�faut de capteur interne (non li� � une d�connection), on switche imm�diatement.  
	...
	// Si les conditions nominales de test proxi vers interne sont respect�s pendant 50 ms, on switche imm�diatement.
	// Si on detecte un d�faut de capteur proximal, on switche imm�diatement.
...
/******************************************************************************/
	...
	// Extrampolation de la pression proximale � partir de la pression interne et de la resistance du circuit
