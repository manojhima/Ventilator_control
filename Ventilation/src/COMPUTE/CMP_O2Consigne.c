...
{
	 // R�cup�ration de la valeur 100% O2	active
	...
	// R�cup�ration du d�bit inspiratoire
  ...
  //Test si 100% O2 demand�
   if (o2_100p100 == TRUE)
   {
	  AdjustFio2 = 100;
	  //R�glage des seuils d'alarmes Low et High
   ...
   //Sinon r�cup�ration de la FIO2 r�gl�e
   else
   {
   ...
   // Non prise en compte d'un d�bit inspir� n�gatif pour la r�gul O2
   if (Qinsp < 0)
   {
      Qinsp = 0;
   }	 
   ...
   // Calcul de la Consigne O2
	//	(Signification du "AdjustFio2+2": Translation de 2% de l'ensemble de la r�gulation des modes � valves
	// pour compenser une faibesse g�n�rale de celle-ci).
...
//////////////////////////////////////////////////////////////////////
	...
	//CORECTION PAR RAPPORT AU %O2 REGLE.
		...
		else
		{
		   ...
		   if ((CurrentMode == VOL) ||
			...
			// Pour les modes Pression � valve (VPC(VPAC)� valve et AI(AIFR) � valve.
			else
			{ 
			 switch (AdjustFio2)
				  {
	...
	// Calcul de la consigne O2 corrig�e: 
	// Int�gration de la Correction fonction du VT et de la correction fonction du %O2 r�gl�.
